...
const railGroups = [
  {
    label: 'Understand',
    sectionIds: ['context', 'orientation', 'recognition'],
  },
  {
    label: 'Decode',
    sectionIds: ['signal-roles', 'pattern-mechanics', 'pattern-synthesis'],
  },
  {
    label: 'Apply',
    sectionIds: ['strengths', 'narrowing', 'application', 'closing-integration'],
  },
] satisfies Array<{ label: string; sectionIds: string[] }>;
      ...
      <div className="draft-reading-rail-card space-y-3 rounded-[1.35rem] border border-[#F3F1EA]/[0.075] bg-[#171D1A]/58 px-3 py-3.5 shadow-[0_16px_36px_rgba(4,7,6,0.12)] backdrop-blur-[14px]">
        ...
        <ul className="sonartra-result-rail-track relative space-y-3 pl-1.5" role="list">
          {railGroups.map((group) => {
            const groupSections = sectionsConfig.topLevelSections.filter((section) =>
