...
test('admin user detail component keeps the operational record layout and wires the safe assignment controls', () => {
  ...
  assert.match(controlsSource, /return 'Editable';/);
  assert.match(controlsSource, /return 'Fixed';/);
  assert.match(controlsSource, /return 'Started';/);
  assert.match(controlsSource, /return 'Completed';/);
  assert.match(controlsSource, /Result ready/);
  assert.match(controlsSource, /Canonical result ready/);
  assert.match(controlsSource, /Queued and safe to move or remove\./);
  assert.match(controlsSource, /Historical row\. Sequence locked\./);
  assert.match(controlsSource, /Execution started\. Sequence locked\./);
  assert.match(controlsSource, /Completed history\. Sequence locked\./);
  assert.match(controlsSource, /Completed with a canonical result\. Sequence locked\./);
