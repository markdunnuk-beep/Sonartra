...
export async function getSingleDomainApplicationStatementRows(
  ...
  return getDatasetRows<ApplicationStatementsRow>({
    ...
    selectColumns: `
      ...
      guidance_text,
      linked_claim_type,
      COALESCE(strength_statement_1, '') AS strength_statement_1,
      COALESCE(strength_statement_2, '') AS strength_statement_2,
      COALESCE(watchout_statement_1, '') AS watchout_statement_1,
      COALESCE(watchout_statement_2, '') AS watchout_statement_2,
      COALESCE(development_statement_1, '') AS development_statement_1,
      COALESCE(development_statement_2, '') AS development_statement_2
    `,
    orderBy: 'domain_key ASC, pattern_key ASC, focus_area ASC, priority ASC, driver_role ASC, signal_key ASC',
  });
}
